#ifndef DMW_IMPL__REQUEST_HPP_
#define DMW_IMPL__REQUEST_HPP_
...
/// Tracks request and response endpoint observations by remote participant.
class RequestState {
public:
...
private:
    ...
    mutable RankedMutex<LockRank::RequestState> mutex_;
...
class RequestWriterListener final : public eprosima::fastdds::dds::DataWriterListener {
public:
    explicit RequestWriterListener(std::weak_ptr<RequestState> state) noexcept
    ...
    void on_publication_matched(
        ...
        if (const auto state = state_.lock()) {
            try {
                state->observe_request_reader(
                    status.last_subscription_handle, status.current_count_change);
            } catch (...) {
                state->degrade();
            }
        }
