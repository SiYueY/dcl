...
Result<std::unique_ptr<Client>> Node::Impl::create_client(
    ...
    auto request_topic =
        impl_->context_state_->acquire_topic(type.request_type(), request_topic_name, qos);
    if (!request_topic)
...
Result<std::unique_ptr<Server>> Node::Impl::create_server(
    ...
    auto request_topic = impl_->context_state_->acquire_topic(
        type.request_type(), service_topic_name(*impl_->context_state_, logical_name.value(), true),
        qos);
    if (!request_topic)
